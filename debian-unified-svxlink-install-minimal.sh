...
if [[ -f /tmp/stage2 ]] && [[ ! -f /tmp/stage3 ]] ; then
        ...
        apt-get install -y --fix-missing libopus0 alsa-utils vorbis-tools sox libsox-fmt-mp3 librtlsdr0 ntp libasound2 \
                libspeex1 libgcrypt20 libpopt0 libopus0 libgsm1 tcl8.6 tk8.6 alsa-base bzip2 flite wvdial inetutils-syslogd \
                screen time uuid vim usbutils dialog logrotate cron gawk watchdog network-manager git-core libsigc++-2.0-0c2a \
                fail2ban resolvconf libasound2-plugin-equal watchdog i2c-tools libxml2 libxml2-dev libssl-dev libxslt1-dev
